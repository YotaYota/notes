...
```js
export default function yourReducerFn(state, action) {
    switch (action.type) {...}
...
const [currentState, dispatch] = useReducer(yourReducerFn, initialState);
...
**Note**: reducers run during rendering (Actions are queued until the next render.)
